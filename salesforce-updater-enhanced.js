...
class SalesforceUpdater {
  ...
  async lookupWorkOrderByNumber(workOrderNumber) {
    try {
      ...
      const soql = `SELECT Id, Name, SASSIE_ID__c, Related_Project__r.SASSIE_Survey_ID__c, Related_Project__r.SASSIE_Survey_Name__c, Related_Project__r.Video_QID__c, Related_Project__r.Download_Video_QID__c FROM Work_Order__c WHERE Name = '${escapedNumber}' LIMIT 1`;
    ...
    try {
      ...
      return await this.updateRecord(
        ...
        updateData,
        {
          ...
          sassieSurveyId: workOrder?.Related_Project__r.SASSIE_Survey_ID__c,
          sassieVideoQID: workOrder?.Related_Project__r.Video_QID__c,
          sassieDownloadQID:
            workOrder?.Related_Project__r.Download_Video_QID__c,
        }
      );
    } catch (error) {
      const failureResult = {
