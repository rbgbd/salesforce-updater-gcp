...
functions.http("salesforceUpdateHandler", async (req, res) => {
  ...
  try {
    ...
    let sfConfig;
    if (useSecretManager) {
      sfConfig = await orchestrator.loadCredentialsFromSecrets(
        projectId,
        secretName
      );
    } else if (config) {
    ...
    if (bucketName && workflowResults.exportResults.files.length > 0) {
      console.log(
        `📤 Uploading ${workflowResults.exportResults.files.length} files to Cloud Storage...`
      );
...
functions.cloudEvent("scheduledSalesforceUpdate", async (cloudEvent) => {
  ...
  try {
    ...
    console.log(
      `✅ Scheduled update completed: ${results.updateResults.successful} successful, ${results.updateResults.failed} failed`
    );
  } catch (error) {
