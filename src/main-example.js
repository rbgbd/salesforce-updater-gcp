...
class SalesforceUpdateOrchestrator {
  ...
  async loadCredentialsFromSecrets(projectId, secretName) {
    try {
      ...
      console.log('🔐 Loaded Salesforce credentials from Google Cloud Secret Manager');
      return credentials;
    } catch (error) {
  ...
  async executeUpdateWorkflow(updates, objectType = null, options = {}) {
    ...
    try {
      ...
      if (Array.isArray(updates)) {
        ...
        results = await this.updater.processUpdates(updates, { batchSize, delayMs });
      } else if (typeof updates === 'object' && objectType) {
        // Key-value pairs
        results = await this.updater.processKeyValueUpdates(objectType, updates, { batchSize, delayMs });
      } else {
        throw new Error('Invalid update data format. Provide either an array of update objects or key-value pairs with objectType.');
      }

      // Export results to CSV
      const exportResults = await CSVExporter.exportUpdateResults(results, exportOptions);
      ...
      console.log(`📊 Summary: ${results.successful} successful, ${results.failed} failed (${results.successRate})`);
